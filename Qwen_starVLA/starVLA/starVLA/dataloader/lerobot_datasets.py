...
def make_LeRobotSingleDataset(
    ...
    # 🌟 伪造白名单：如果碰到不认识的 piper，就借用白名单里已有的第一个配置垫背
    if robot_type == 'piper' or robot_type not in ROBOT_TYPE_CONFIG_MAP:
        robot_type = list(ROBOT_TYPE_CONFIG_MAP.keys())[0]
...
def get_vla_dataset(
    ...
    **kwargs: dict,
    ...
    if data_mix in DATASET_NAMED_MIXTURES:
        ...
        print(f"\n⚠️ [黑客模式] 发现未知数据集 '{data_mix}'！绕过白名单，强行加载...")
        # 智能判断：如果是官方 Demo 就用 franka_robotiq，否则全默认为 piper！
        robot_t = "franka_robotiq" if "pick_place" in data_mix else "piper"
        # 强行伪造白名单格式：[(数据集文件夹名, 权重比例, 机器人类型)]
        mixture_spec = [(data_mix, 1.0, robot_t)]
    # ==========================================
