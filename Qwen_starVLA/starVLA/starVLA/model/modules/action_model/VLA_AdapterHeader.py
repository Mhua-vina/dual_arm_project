...
class VLA_Adapter_L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.input_dim = input_dim  # Store input_dim as instance variable
        ...
        self.action_chunk_embeddings = nn.Parameter(
            torch.zeros(self.num_actions_chunk, action_dim * input_dim)
        )
        nn.init.normal_(self.action_chunk_embeddings, mean=0.0, std=0.02)
        
        self.model = MLPResNet(
            num_blocks=6,  # 减少blocks数量以节省显存，原为24
    ...
    def predict_action(
        ...
        cond_actions_hidden_states = torch.zeros(
            (batch_size, self.action_dim * self.num_actions_chunk, self.input_dim),
...
class MLPResNet(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            if use_pro_version:
                # For MLPResNetBlock_Pro, dim should be input_dim because h_a, h_t, p are input_dim
                self.mlp_resnet_blocks.append(MLPResNetBlock_Pro(dim=input_dim))
            else:
                # For MLPResNetBlock, dim should be input_dim because h_a, h_t, p are input_dim
                self.mlp_resnet_blocks.append(MLPResNetBlock(dim=input_dim))
