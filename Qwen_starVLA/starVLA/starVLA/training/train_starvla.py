3# Copyright 2025 starVLA community. All rights reserved.
...
class VLATrainer(TrainerUtils):
    ...
    def _enable_gradient_checkpointing(self):
        """启用 gradient checkpointing 以减少训练时的显存占用"""
        enabled_count = 0
        skipped_count = 0
        
        # 遍历所有子模块，启用 gradient checkpointing
        for name, module in self.model.named_modules():
            try:
                ...
                # 某些模块不支持 gradient checkpointing，跳过
                skipped_count += 1
                continue
            try:
        ...
        logger.info(f"Gradient checkpointing: enabled={enabled_count}, skipped={skipped_count}")
