...
class TrainerUtils:
    ...
    def freeze_backbones(model, freeze_modules=""):
        """
        Freeze submodules based on regex patterns or relative module path list.
        Supports both:
          - Regex patterns like ".*qwen.*|.*vlm.*" (matches module names containing qwen or vlm)
          - Relative path list like "qwen_vl_interface, action_model.net"
        """
        frozen = []
        print("#"*30)
        ...
        if freeze_modules and isinstance(freeze_modules, str):
            ...
            for name, module in model.named_modules():
                ...
                for path in patterns:
                    # If pattern contains regex meta-characters, use regex match
        ...
        if dist.is_initialized() and dist.get_rank() == 0:
            print(f"🔒 Frozen modules: {len(frozen)} modules matched")
        return model
